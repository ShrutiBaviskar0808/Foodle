...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  Future<void> _loadData() async {
    await Future.wait([_loadFamilyMembers(), _loadPlaces(), _loadFriends()]);
  }
                        ...
                        child: CircleAvatar(
                          ...
                          child: FutureBuilder<String>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              return Text(
                                initial,
                                style: const TextStyle(color: Color(0xFFFE8D00), fontSize: 20, fontWeight: FontWeight.bold),
                              );
                            },
                          ),
                        ),
      ...
      children: [
        CircleAvatar(
          ...
          child: imagePath == null || (!imagePath.startsWith('http') && !File(imagePath).existsSync())
              ? Text(name.isNotEmpty ? name[0].toUpperCase() : '?', style: const TextStyle(fontSize: 24, color: Colors.orange, fontWeight: FontWeight.bold))
              : null,
        ),
      ...
      children: [
        CircleAvatar(
          ...
          backgroundImage: imagePath != null && imagePath.startsWith('http')
              ...
              : (imagePath != null && File(imagePath).existsSync() ? FileImage(File(imagePath)) : null) as ImageProvider?,
          child: imagePath == null || (!imagePath.startsWith('http') && !File(imagePath).existsSync())
              ? Text(name.isNotEmpty ? name[0].toUpperCase() : '?', style: const TextStyle(fontSize: 24, color: Colors.grey, fontWeight: FontWeight.bold))
              : null,
        ),
        const SizedBox(height: 8),
        SizedBox(
